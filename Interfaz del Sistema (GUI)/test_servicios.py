...
# IMPORTAR EL MÓDULO PRINCIPAL - CORREGIDO
try:
    import gestor_proyectos as m  # Cambia al nombre de tu archivo principal
except ImportError:
    try:
        import main as m
    except ImportError:
...
class TestValidaciones(unittest.TestCase):
    ...
    def test_02_validar_fecha_formato_incorrecto(self):
        ...
        self.assertFalse(m.validar_fecha("32/01/2026"))  # Día inválido
        self.assertFalse(m.validar_fecha("15/13/2026"))  # Mes inválido
...
class TestRegistroHoras(unittest.TestCase):
    ...
    def test_26_no_superar_24h_mismo_dia(self):
        ...
        # Verificar que 20 + 5 > 24 (no se puede agregar 5 más)
        self.assertTrue(acum + 5.0 > 24.0)
        # Verificar que 20 + 4 <= 24 (sí se puede agregar 4)
        self.assertTrue(acum + 4.0 <= 24.0)
    ...
    def test_29_horas_negativas_invalidas(self):
        """Las horas negativas deben ser rechazadas"""
        horas = -1.0
        self.assertTrue(horas <= 0)  # Esto indica que es inválido

    def test_30_horas_cero_invalidas(self):
        """Las horas cero deben ser rechazadas"""
        horas = 0.0
        self.assertTrue(horas <= 0)  # Esto indica que es inválido
    ...
    def test_35_eliminar_tarea_con_registros(self):
        ...
        while i < len(m.reg_tarea):
            if m.reg_tarea[i] == idx:
                ...
                if i < len(m.reg_tarea):
                    m.reg_tarea.pop(i)
                if i < len(m.reg_empleado):
                    m.reg_empleado.pop(i)
                if i < len(m.reg_fecha):
                    m.reg_fecha.pop(i)
                if i < len(m.reg_horas):
                    m.reg_horas.pop(i)
                # No incrementar i porque los elementos se desplazan
            else:
                if m.reg_tarea[i] > idx:
                    m.reg_tarea[i] -= 1
                i += 1
        ...
        # Eliminar la tarea
        if len(m.nom_tarea) > 0:
            m.nom_tarea.pop(0)
        if len(m.tarea_proyecto) > 0:
            m.tarea_proyecto.pop(0)
        if len(m.tarea_empleado) > 0:
            m.tarea_empleado.pop(0)
